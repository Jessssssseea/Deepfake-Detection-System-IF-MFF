...
def main():
    parser = argparse.ArgumentParser(description='筛选高质量且不重复的人脸图片')
    ...
    if len(results) > 0:
        ...
        for score, emb, path in results:
            if len(selected) >= num_keep:
                break
            if len(embeddings_selected) == 0:
                max_sim = 0
            else:
                sims = cosine_similarity([emb], embeddings_selected)[0]
                max_sim = np.max(sims)
            if max_sim < sim_thresh:
                selected.append((score, emb, path))
                embeddings_selected.append(emb)
                kept_paths.append(path)

        print(f"去重后共保留 {len(kept_paths)} 张图片")
