...
def extract_features_from_image(img_bgr):
    ...
    mouth_mean = np.mean(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)[mouth_mask==255], axis=0) if np.any(mouth_mask) else np.zeros(3)
    ...
    def update_expected_frames(self):
        ...
        if self.full_checkbox.isChecked():
            ...
            if manual_num > total_frames:
                self.expected_label.setText(f"注意：抽帧数({manual_num})大于总帧数({total_frames})，将抽取全部 {total_frames} 帧")
            else:
    ...
    def browse_video(self):
        ...
        if path:
            ...
            if ret:
                ...
                scaled = pixmap.scaled(self.video_preview_label.width(), 150, Qt.KeepAspectRatio, Qt.SmoothTransformation)
